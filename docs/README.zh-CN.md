...
- OpenCode 等待权限审批时提供“仅允许这次”“持续允许同类操作”“拒绝”按钮。
...
| `/stop` | 二次确认后停止当前任务 |
| `/approvals` | 重新显示尚未处理的 OpenCode 审批 |
...
OpenCode 因文件修改、命令执行等操作等待审批时，机器人会显示权限类型、目标和项目目录。点击按钮后由桥接调用 OpenCode 本机审批接口，任务继续运行。桥接重启后会重新发现尚未处理的审批，也可以随时发送 `/approvals` 重新显示。
...
- 审批按钮使用短本地映射编号，不包含命令正文、会话 ID 或认证信息；重复点击不会重复执行。
